...
class AgentConfig(object):
    ...
    def __init__(self, curr_step_ind, system_config):
        ...
        if curr_step_ind == 0:
            ...
            self.action_bounds = np.array([[-3500, 3500],  # Rotor 1 RPM
                                           [-3500, 3500],  # Rotor 2 RPM
                                           [-3500, 3500],  # Rotor 3 RPM
                                           [-3500, 3500]]) # Rotor 4 RPM
...
class ExcitationSignalConfig(object):
    ...
    def __init__(self, curr_step_ind):
        ...
        # Altitude control
            ...
            self.means = np.array([0.0, 0.0, 0.0, 0.0])
            self.amplitudes = np.array([70.5, 10.0, 10.0, 21.0])
            self.frequency_ranges = np.array([[30, 100],  # [m/s]
                                              [50, 100],  # [rad/s]
                                              [50, 100],  # [rad/s]
                                              [50, 100]]) # [rad/s]
...
class ReferenceSignalConfig(object):
    ...
    def __init__(self, curr_step_ind, dt):
        ...
        if curr_step_ind == 0:
            ...
            self.signal_type = 'sinusoidal'
            self.amplitudes = np.array([np.pi/18, np.pi/24, np.pi/24, 1.0])
            # self.phase = np.random.random() * np.pi
            self.phases = np.array([np.pi/2, np.pi/2, np.pi/2, np.pi/2])
            self.frequencies = np.array([2, 3, 5, 0.3]) * 1/(2*np.pi)
            ...
            # self.phase = np.random.random() * np.pi
            self.phases = np.array([np.pi/2, np.pi/2, np.pi/2, np.pi/2])
            self.frequencies = np.array([2, 4, 5, 0.8]) * 1/(2*np.pi)
...
class SafetyFilterConfig(object):
    ...
    def __init__(self, curr_step_ind, dt, action_bounds):
        ...
        if self.activated:
            ...
            self.policy_search_processing = 'sequential'  # or ('s') also 'parallel' or ('p')
...
class SupervisorConfig(object):
    ...
    $
        K[0, 8] = -1.00486065074353e+00  ## -1.178418572021728e+00  # Cancel vertical rate (w)
        K[0,11] = -3.15234749356518e-01  ##-0.315380836401326e+00  # Cancel vertical position (z)

        K[1, 0] =  3.73270718306453e+00  ## 1.4996e+00  # Cancel roll (phi)
        K[1, 3] =  5.82323658308057e-01  ## 4.3504e-01  # Cancel roll rate (p)
        K[1, 7] =  4.75668231057331e-01  ## 3.0715e-01  # Cancel y-velocity (v)
        K[1,10] =  2.96066510623104e-01  ## 3.0542e-01  # Cancel y-position (y)

        K[2, 1] =  3.73270718306446e+00  ## 1.5192e+00  # Cancel pitch (theta)
        K[2, 4] =  5.82323658308098e-01  ## 4.4361e-01  # Cancel pitch rate (q)
        K[2, 6] = -4.75668231057082e-01  ## -3.0916e-01  # Cancel x-velocity (u)
        K[2, 9] = -2.96066510623152e-01  ## -3.0549e-01  # Cancel x-position (x)

        K[3, 2] =  3.03291618795145e+00  ## 9.9374e-02  # Cancel yaw angle (psi)
        K[3, 5] =  7.41784860804882e-01  ## 1.6062e-01  # Cancel yaw rate (r)
    ...
    def __init__(self, curr_step_ind):
        ...
        if curr_step_ind == 0:
            ...
            # # Define Gain matrix
            K = np.zeros((3,10))
            K[0,6] = -1.00486065074353e+00  # Cancel vertical rate (w)
            K[0,7] = -3.15234749356518e-01  # Cancel vertical position (z)
            K[1,0] =  3.73270718306453e+00  # Cancel roll (phi)
            K[1,3] =  5.82323658308057e-01  # Cancel roll rate (p)
            K[2,1] =  3.73270718306446e+00  # Cancel pitch (theta)
            K[2,4] =  5.82323658308098e-01  # Cancel pitch rate (q)
            ...
            K[0,6] = -1.00486065074353e+00  # Cancel vertical rate (w)
            K[0,7] = -3.15234749356518e-01  # Cancel vertical position (z)
            K[1,1] =  3.73270718306446e+00  # Cancel pitch (theta)
            K[1,4] =  5.82323658308098e-01  # Cancel pitch rate (q)
